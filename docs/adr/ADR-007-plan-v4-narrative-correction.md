# ADR-007 — Plan-v4: narrative correction + six-week closure plan
...
2. **The implicit title assumption is wrong.** Plan-v3 framed the architect as already holding a Platform Engineer title. The day-job title is Technical Operations Engineer at a UK MSP; the capability the portfolio demonstrates is broader than the day-job title. Plan-v3 collapsed those into one, which produced positioning language that either overclaimed (asserting the title) or undersold (treating the gap as "transitioning"). Both modes are corrected in Plan-v4: the portfolio is the evidence, on the record, and the framing stops trying to assert or transition anything.
3. **No bridge document.** Plan-v3 has no artefact that translates the engineering depth into a scoped reading path. The ADRs and runbooks are findable to someone who already knows the repo; they are invisible to a reader scanning cold. The engineering work is there; the framing is not.

Plan-v4 corrects all three. The cost of the correction is the planning artefact itself (this ADR, the operating doc at `docs/plan-v4/README.md`, the bridge document at `BLS-PLATFORM-ENGINEERING-GUIDE.md`, the concept→tool mapping at `docs/plan-v4/concept-tool-mapping.md`, and the diagram catalogue). The benefit is a portfolio that is shippable in four weeks instead of fourteen.
...
Continue under Plan-v3's framing; treat the existing build as Weeks 0–8 retroactively; add a bridge document as a Plan-v3 Week 19 deliverable.

**Rejected.** The build schedule is wrong; an extension does not fix it. Continuing the eighteen-week framing implies eight more weeks of construction before the portfolio is shippable, which is incorrect — the construction is largely done; the polish and positioning is what remains. Bolting a bridge document onto a misaligned framing produces a bridge document that inherits the misalignment.
...
**Rejected.** Demonstrated platform capability outweighs certifications for the work this portfolio surfaces. The portfolio already demonstrates the capability; certifications without the portfolio are weaker than the portfolio without additional certifications. AZ-104 is in progress in the background at its own pace and continues; it is not on the Plan-v4 critical path. A pause is the wrong response to "the engineering work is ahead of the planning narrative."
...
**Rejected.** A twelve-week plan does not produce a meaningfully better portfolio than a six-week plan because the marginal bridge-content and diagram additions are subject to diminishing returns. The hiring signal does not turn on whether the portfolio has six diagrams or ten; it turns on whether the engineering depth is visible and the framing is honest. Six weeks is long enough to land both. Twelve weeks costs six additional weeks of application throughput.
...
Bridge document and diagram catalogue are first-class deliverables. Concept-first, tool-second framing applied uniformly across every artefact.

---

## Decision

**Adopt Plan-v4: six-week narrative + closure plan, monorepo retained, bridge document plus diagram catalogue, concept-first framing.**
...
Time horizon: six weeks. Success criterion: an active application pipeline backed by a shippable, end-to-end-legible portfolio by the end of Week 6. Reassessment trigger: if Week 6 closes with no pipeline activity, the plan exits with a written retrospective on what the six weeks produced, what worked, what did not, and which assumptions need revising before the next attempt. No mid-plan replacement.
...
**A bridge document is load-bearing for legibility.** Engineering depth without a reading path is invisible to a cold reader. Six ADRs and two runbooks distributed across `docs/adr/` and `docs/runbooks/` are findable to someone who already knows the repo; they are invisible to anyone scanning for five minutes. The bridge document provides the reading path.
...
**Six weeks is calibrated against application-pipeline throughput.** A four-week plan would leave Week 4–6 of the calendar without a written gate; a twelve-week plan would consume application weeks on diminishing-return polish. Six weeks lands "shippable" at Week 4 Day 1 and reserves Weeks 5–6 for application throughput plus interview load.

**Concept-first framing is a signal multiplier.** A reader picks the candidate who can name the architectural concept before the tool that implements it. The concept→tool mapping table makes this discipline structural — every artefact links to the same table; framing drift across artefacts is mechanically caught.
...
- **Faster path to a shippable portfolio.** Six weeks of polish-and-position instead of fourteen of construction-plus-polish.
...
- **Bridge document removes the "scan for five minutes and bounce" failure mode.** A cold reader's first five minutes now lands them on a concept-first reading path with linked depth.
...
- **Plan-v3 historical artefacts remain reachable via `git log`.** The repository is not being rewritten. A reader walking the history will see commits from the Plan-v3 era with framings that Plan-v4 supersedes. This is an honesty surface, not a defect.
...
| Alternative | Reason |
|---|---|
| Extend Plan-v3 in place | Inherits the wrong build-schedule framing; bolting a bridge document onto misaligned framing does not fix the misalignment. |
...
| Pause portfolio for certifications | Demonstrated capability outweighs certifications for the surface this portfolio covers; the portfolio already demonstrates the capability; pausing trades a strength for a weaker substitute. |
| Twelve-week extended plan | Marginal polish-and-diagram additions are diminishing returns; six additional weeks of unemployment outweighs the marginal portfolio gain. |
...
This ADR is reviewed at end of Week 6 against the success criterion (an active application pipeline backed by a shippable portfolio). If the criterion is met, the ADR's Status stays Accepted; the retrospective links from here. If not, the retrospective drives the next ADR (ADR-008) that supersedes this one.
